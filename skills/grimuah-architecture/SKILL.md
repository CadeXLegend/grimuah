...
7. Place the innate members inside the owning surface, never in a central tree. `<name>.types.ts` holds its contracts, `<name>.config.ts` holds its constants and enums, `<name>.spec.ts` holds its tests, `<name>.regex-patterns.ts` holds its documented patterns. A config earns its file rather than assuming one: the rules that name a `.config.ts` as the destination count the entries the file would hand it, and fewer than four is a second file to open for constants the module already declares, so a constant or two and a small enum stay where they are. A config the surface already holds takes them either way. A `config/`, `types/` or `models/` directory under `src/` is flagged by name. An innate member inherits its surface's `dagOrder` and may not import from a deeper surface, because that would drag implementation detail into a contract.
